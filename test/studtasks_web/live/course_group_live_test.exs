defmodule StudtasksWeb.CourseGroupLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all course_groups", %{conn: conn, course_group: course_group} do
      {:ok, _index_live, html} = live(conn, ~p"/course_groups")
    ...
    test "saves new course_group", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/course_groups")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Course group")
               |> render_click()
               |> follow_redirect(conn, ~p"/course_groups/new")
      ...
      assert {:ok, index_live, _html} =
               ...
               |> render_submit()
               |> follow_redirect(conn, ~p"/course_groups")
    ...
    test "updates course_group in listing", %{conn: conn, course_group: course_group} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/course_groups/#{course_group}/edit")
  ...
  describe "Show" do
    ...
    test "displays course_group", %{conn: conn, course_group: course_group} do
      {:ok, _show_live, html} = live(conn, ~p"/course_groups/#{course_group}")
    ...
    test "updates course_group and returns to show", %{conn: conn, course_group: course_group} do
      {:ok, show_live, _html} = live(conn, ~p"/course_groups/#{course_group}")

      assert {:ok, form_live, _} =
               show_live
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/course_groups/#{course_group}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#course_group-form", course_group: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/course_groups/#{course_group}")
